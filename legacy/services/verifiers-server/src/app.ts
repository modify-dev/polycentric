...
import * as Core from '@polycentric/legacy-polycentric-core';
import { createPersistenceDriverLevelDB } from './persistence-driver-leveldb';
...
async function loadProcessHandle(): Promise<Core.ProcessHandle.ProcessHandle> {
  const persistenceDriver = createPersistenceDriverLevelDB('./state');
