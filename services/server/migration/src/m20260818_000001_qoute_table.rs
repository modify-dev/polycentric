...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        if manager.has_table(quote_model::Entity.unquoted()).await? {
            return Ok(());
        }
        ...
        create_table
            .table(quote_model::Entity.unquoted())
            .col({
                let mut def = ColumnDef::new(quote_model::Column::EventId);
                def.primary_key().big_integer().not_null();
                def
            })
            .col({
                let mut def = ColumnDef::new(quote_model::Column::Identity);
                def.text().not_null();
                def
            })
            .col({
                let mut def = ColumnDef::new(quote_model::Column::Post);
        ...
        fill_table
            .into_table(quote_model::Entity)
            .columns([
                quote_model::Column::EventId,
                quote_model::Column::Identity,
                quote_model::Column::Post,
            ])
            .select_from({
                ...
                q
                    ...
                    .expr(SelectExpr {
                        ...
                        alias: Some(quote_model::Column::EventId.into()),
                        window: None,
                    })
                    .expr(SelectExpr {
                        expr: Expr::col(event_model::Column::Identity.as_column_ref()),
                        alias: Some(quote_model::Column::Identity.into()),
                        window: None,
                    })
                    .expr(SelectExpr {
                        expr: Expr::col((
                              "quote_event", event_model::Column::Id.unquoted(),
                        )),
                        alias: Some(quote_model::Column::Post.into()),
                        window: None,
                    })
                    ...
                    .inner_join(
                        TableRef::Table(event_model::Entity.into(), Some("quote_event".into())),
                        Condition::all()
                            .and(Expr::col(content_post_model::Column::QuoteCollection.as_column_ref())
                                .eq(Expr::col(("quote_event", event_model::Column::Collection))))
                            .and(Expr::col(content_post_model::Column::QuoteIdentity.as_column_ref())
                                .eq(Expr::col(("quote_event", event_model::Column::Identity))))
                            .and(Expr::col(content_post_model::Column::QuotePublicKeyType.as_column_ref())
                                .eq(Expr::col(("quote_event", event_model::Column::PublicKeyType))))
                            .and(Expr::col(content_post_model::Column::QuotePublicKey.as_column_ref())
                                .eq(Expr::col(("quote_event", event_model::Column::PublicKey))))
                            .and(Expr::col(content_post_model::Column::QuoteSequence.as_column_ref())
                                .eq(Expr::col(("quote_event", event_model::Column::Sequence))))
                    )
                    .left_join(
            ...
            .on_conflict({
                let mut c = OnConflict::column(quote_model::Column::EventId);
                c.do_nothing();
                c
            });
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        drop_table.table(quote_model::Entity.unquoted()).restrict();
