...
import { PolycentricClient, KEY_TYPE } from '@polycentric/js-core';
...
import { ClaimFieldEntry, SignedEvent } from '@polycentric/js-core';

import { TEST_DB_DIR, deleteDatabase } from './utils';
...
describe('PolycentricClient Claims and Vouches', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      await deleteDatabase(TEST_DB_NAME_1);
      await deleteDatabase(TEST_DB_NAME_2);
    } catch {
    ...
    const clientConfig1: PolycentricClientConfig = {
      ...
      storageDriver: await SqlStorageDriver.create(TEST_DB_NAME_1, TEST_DB_DIR),
      cryptoManager: new NodeCryptoManager(),
    };

    const clientConfig2: PolycentricClientConfig = {
      coreBridge: new NodeWasmBridge(),
      storageDriver: await SqlStorageDriver.create(TEST_DB_NAME_2, TEST_DB_DIR),
      cryptoManager: new NodeCryptoManager(),
    };
